...
from openpoints.AMContrast3D.MaskedRefine import RefinementMethod
...
class PointNextEncoder_M_AMContrast3D(nn.Module):
    def __init__(self,
                 in_channels: int = 4,
                 width: int = 32,
    ...
    def forward_seg_feat_ACE(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            ...
            if i != len(self.encoder)-1:
                _p_flatten = torch.flatten(_p, start_dim=0, end_dim=1)                  ### [b*24000, 3]
                _f_flatten = torch.flatten(_f.transpose(1, 2), start_dim=0, end_dim=1)  ### [b*24000, 64]
                _o_flatten = torch.IntTensor([_p_flatten.shape[0]]).cuda()              ### [b*24000]
...
@MODELS.register_module()
class PointNextDecoder_M_AMContrast3D(nn.Module):
    def __init__(self,
    ...
    def forward_then_AMContrast3D(self, p, f, stage_list, mapping, attention, concate, nsample_k, threshold, threshold_max, gamma, fusion, num_classes, ignore_index, ambiguity_args):

        ''' =====> Ambiguity List: AEF & APM'''
        if (ambiguity_args.source == 'APM' and ambiguity_args.source_mode == 'Train') or (ambiguity_args.source == 'APM' and ambiguity_args.source_mode == 'Test'):
            a_list = stage_list['ambiguity']
            a_cat = torch.cat(a_list)
            # print('APM (max~mean~min):', round(torch.max(a_cat).item(),2), round(torch.mean(a_cat).item(),2), round(torch.min(a_cat).item(),2))
        elif ambiguity_args.source == 'AEF' and ambiguity_args.source_mode == 'Train':
            if 'y' not in stage_list['inputs'].keys():
                raise ValueError('Please change [ambiguity_args.source] to be [APM] for [Test].')
            a_list = self.ambiguity_head(stage_list['inputs']['y'].flatten(), stage_list, num_classes, ignore_index, ambiguity_args)
            a_cat = torch.cat(a_list)
            # print('AEF (max~mean~min):', round(torch.max(a_cat).item(),2), round(torch.mean(a_cat).item(),2), round(torch.min(a_cat).item(),2))
            stage_list['ambiguity_GT'] = a_list
        else:
            raise ValueError('Please change [ambiguity_args.source] to be [APM] for [Test].')


        '''=====> Decoding Stage: -1, -2, -3, -4'''
        refine_rate = []

        for i in range(-1, -len(self.decoder) - 1, -1):  

            '''1. [PointNeXt] Feature Embedding: i-1 = -2, -3, -4, -5'''
            ...
            '''2. [AMContrast3D] Update Embedding for Contrastive Objective: Up-Sampling 0,1,2,3 == -4,-3,-2,-1'''
            stage_list['up'][i]['f_out'] = _f_flatten

            '''3. [AMContrast3D++] MaskedRefine'''
            B = f[0].shape[0]
            a = a_list[i].unsqueeze(0).view(B, 1, -1) 
            refine = RefinementMethod(stage_list, p[i-1], f[i-1], a, i, B, nsample_k, fusion, threshold_max, threshold, gamma)
            if mapping == True:
                if attention == True:  
                    f[i-1] = refine.MapAttention()
                elif attention == False:
                    f[i-1] = refine.MapSum()
            elif mapping == False:
                f[i-1], rate = refine.DualMasks()
                refine_rate.append(rate)

        AVG_refine_rate = np.mean(refine_rate)

        '''Decoder Layer: -5'''
        f_output = f[-len(self.decoder) - 1].squeeze(-1)
        return f_output, stage_list, AVG_refine_rate


    def forward(self, p, f, stage_list, mapping, attention, concate, nsample_k, threshold, threshold_max, gamma, fusion, num_classes, ignore_index, ambiguity_args):
        '''p,f = [0,1,2,3,4,5]'''
        '''stage_list: 'up', 'down', 'ambiguity'= [1,2,3,4]'''
        return self.forward_then_AMContrast3D(p, f, stage_list, mapping, attention, concate, nsample_k, threshold, threshold_max, gamma, fusion, num_classes, ignore_index, ambiguity_args)
