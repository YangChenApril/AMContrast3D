...
def get_features_by_keys(data, keys='pos,x'):
    key_list = keys.split(',')
    ###
    #print(data['pos'].shape) # torch.Size([6, 24000, 3])
    #print(data['x'].shape) ### [xyz] # torch.Size([6, 24000, 3])
    #print(data['heights'].shape) # torch.Size([6, 24000, 1])
    #print(data['y'].shape) ### [Target] # torch.Size([6, 24000])
    #print(data)
    ###
    if len(key_list) == 1:
