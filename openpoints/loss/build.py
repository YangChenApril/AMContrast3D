...
from openpoints.AMContrast3D.MarginContrast import ContrastHead
...
def MAPE(output, target):
    _eps = 1e-12
    return torch.mean(torch.abs((output - target) / (target + _eps)))

@LOSS.register_module()
class CrossEntropyAcePre(torch.nn.Module):
    def __init__(self, **kwargs):
        super(CrossEntropyAcePre, self).__init__()
        '''Segmentation'''
        self.creterion = CrossEntropyLoss()
        '''AEF'''
        self.contrast_head = ContrastHead()
        '''APM'''
        self.MAE = nn.L1Loss()
        self.MSE = nn.MSELoss()
        self.HUBER = nn.HuberLoss(reduction='mean', delta=0.1)

    def forward(self, logit, target, stageACE_list, num_classes, ignore_index, ambiguity_args): 
        ''''logit': label Pre. [batch_size, 13, points_num]: [6, 13, 24000] --> [144000, 13]
           'target': label G.T. [batch_size, 1, points_num]: [6, 24000] --> [144000]
           'stageACE_list': ['inputs', 'down', 'up', 'ambiguity']]'''

        logit = logit.transpose(1, 2).reshape(-1, logit.shape[1])
        target = target.flatten()      

        CrossEntropy_Loss = self.creterion(logit, target)
        Contrastive_Loss, target_ai, _ = self.contrast_head(logit, target, stageACE_list, num_classes, ignore_index, ambiguity_args)

        logits_ai = torch.cat(stageACE_list['ambiguity']).flatten()

        Regression_Loss = self.MAE(logits_ai, target_ai)

        w1 = ambiguity_args.w1
        w2 = ambiguity_args.w2
        w3 = ambiguity_args.w3

        CrossEntropy_Loss = w1 * CrossEntropy_Loss
        Contrastive_Loss = w2 * Contrastive_Loss
        Regression_Loss = w3 * Regression_Loss

        Segmentation_loss = CrossEntropy_Loss + Contrastive_Loss
        
        return Segmentation_loss, CrossEntropy_Loss, Contrastive_Loss, Regression_Loss




@LOSS.register_module()
class CrossEntropyAce(torch.nn.Module):
    def __init__(self, **kwargs):
        super(CrossEntropyAce, self).__init__()
        self.creterion = CrossEntropyLoss()
        self.contrast_head = ContrastHead()
    ...
    def forward(self, logit, target, stageACE_list, num_classes, ignore_index, ambiguity_args): 
        ''''logit': label Pre. [batch_size, 13, points_num]: [6, 13, 24000] --> [144000, 13]
           'target': label G.T. [batch_size, 1, points_num]: [6, 24000] --> [144000]
           'stageACE_list': print(stageACE_list.keys()) # ['inputs', 'down', 'up']]'''
        ...
        w1 = ambiguity_args.w1
        w2 = ambiguity_args.w2
        Segmentation_loss = w1 * CrossEntropy_Loss + w2 * Contrastive_Loss
